...
#log values with quadratic 
par(mfrow=c(2,2))
plot(horse.model3)
summary(horse.model3)
#log values without quadratic 
par(mfrow=c(2,2))
plot(horse.model4)
...
library(leaps)
golf<-read.csv("~/Documents/RashmiDocuments/MSDS@SMU/Summer2021/6372Stats2/Unit2/Unit2PreLive/GolfData2.csv",header=T,stringsAsFactors = TRUE)
...
set.seed(1234)
index<-sample(1:dim(new.Auto)[1],193,replace=F)
train.data<-new.Auto[index,]
test.data<-new.Auto[-index,]
auto.model=regsubsets(log(mpg)~.,data=train.data,method="forward",nvmax=7)
summary(auto.model)$rss
predict.regsubsets =function (object , newdata ,id ,...){
  form=as.formula (object$call [[2]])
  mat=model.matrix(form ,newdata )
  coefi=coef(object ,id=id)
...
#note my index is to 20 since that what I set it in regsubsets
for (i in 1:7){
  predictions<-predict.regsubsets(object=auto.model,newdata=test.data,id=i) 
  testASEauto[i]<-mean((log(test.data$mpg)-predictions)^2)
}
str(testASEauto)

par(mfrow=c(1,1))
plot(1:7,testASEauto,type="l",xlab="# of predictors",ylab="test vs train ASE",ylim=c(0.3,0.8))
index<-which(testASEauto==min(testASEauto))
points(index,testASEauto[index],col="red",pch=10)
rss<-summary(auto.model)$rss
lines(1:7,rss/193,lty=3,col="blue")  #Dividing by 100 since ASE=RSS/sample size
#########################
library(ggvis)
rss <- as.data.frame(auto.model$rss/193)
names(rss) <- "R2"
rss %>% 
        ggvis(x=~ c(1:nrow(rss)), y=~R2 ) %>%
        layer_points(fill = ~ R2 ) %>%
        add_axis("y", title = "R2") %>% 
        add_axis("x", title = "Number of variables")


plot(auto.model$rss ,xlab="Number of Variables ",ylab="RSS",type="l")
index<-which(rss==min(auto.model$rss))
points(index,auto.model$rss[index],col="red",pch=10)



library(tidyverse)
ggplot(new.Auto,aes(x=))
...
# Splitting data
str(new.Auto)
new.Auto$cylinders=as.integer(new.Auto$cylinders)
new.Auto$origin=as.integer(new.Auto$origin)
new.Auto$mpg=as.integer(new.Auto$mpg)
new.Auto$displacement=as.integer(new.Auto$displacement)
new.Auto$horsepower=as.integer(new.Auto$horsepower)
new.Auto$weight=as.integer(new.Auto$weight)
new.Auto$acceleration=as.integer(new.Auto$acceleration)
new.Auto$year=as.integer(new.Auto$year)
library(tidyverse)
head(Auto)
new.Auto=Auto%>%filter(cylinders!=3)%>%filter(cylinders!=5)
dim(new.Auto)
library(caret)
...
train.data<-new.Auto[index,]
test.data<-new.Auto[-index,]
model=lm(log(mpg)~displacement+horsepower+weight+acceleration+year+origin,data=train.data)
summary(model)
...
for (i in 1:20){
  predictions<-predict(model,newdata=test.data) 
  testASE[i]<-mean((log(test.data$mpg)-predictions)^2)
}
par(mfrow=c(1,1))
plot(1:20,testASE,type="l",xlab="# of predictors",ylab="test vs train ASE",ylim=c(0.3,0.8))
index<-which(testASE==min(testASE))
...
lines(1:20,rss/193,lty=3,col="blue")  #Dividing by 100 since ASE=RSS/sample size
dim(train.data)
...
plot(1:20,predictions)
length(model$rss)
corrplot::corrplot(cor(new.Auto),method="number")
