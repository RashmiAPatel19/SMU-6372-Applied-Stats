...
corrplot::corrplot(cor(Auto),method="number")
```
...
auto.model=regsubsets(log(mpg)~displacement+horsepower+weight+acceleration+year+origin,data=train.data,method="forward",nvmax=7)
summary(auto.model)$rsq
predict.regsubsets =function (object , newdata ,id ,...){
  form=as.formula (object$call [[2]])
  mat=model.matrix(form ,newdata )
  coefi=coef(object ,id=id)
...
testASEauto
par(mfrow=c(1,1))
plot(1:7,testASEauto,type="l",xlab="# of predictors",ylab="test vs train ASE",ylim=c(0,0.2))
...
dim(train.data)
# final model
reg.final=regsubsets(log(mpg)~displacement+horsepower+weight+acceleration+year+origin,data=train.data,method="forward",nvmax=7)
coef(reg.final,3)
##  (Intercept)       weight         year      origin3 
## 1.659606737 -0.000286848  0.029865652  0.082995698 

final.model<-lm(log(mpg)~weight+year+origin,data=new.Auto)
summary(final.model)


plot(exp(final.model$fitted.values),new.Auto$mpg,xlab="Predicted",ylab="mpg",xlim=c(0,100),ylim=c(0,100))
